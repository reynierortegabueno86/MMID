...
from datamodule import MultiIronyDataModule
from models import DualTransformerModel
...
if __name__ == "__main__":
    ...
    print(parameters.model,fileoutname)
    sql3.close()
    ...
    ironyTrainID, ironyTrainLabels, ironyDataTrain = ironyDataTrain["id"], ironyDataTrain["irony"], ironyDataTrain[["irony", "preprotext", 'images']]
    ...
    ironyTestID, ironyTestLabels, ironyDataTest = ironyDataTest["id"], ironyDataTest["irony"], ironyDataTest[["irony", "preprotext",'images']]
    ...
    ironyValID, ironyValLabels, ironyDataVal = ironyDataVal["id"], ironyDataVal["irony"], ironyDataVal[["irony", "preprotext",'images']]
    vallab = ironyValLabels.to_numpy()

    feature_extractor = ViTFeatureExtractor.from_pretrained('google/vit-base-patch16-224')

    overallACC, overallF1, history = 0.0, 0.0, []
    ...
    with open(parameters.output + os.sep + fileoutname, 'w')  as myout:
        ...
        print("#" * 50)
        print("TRAINING STAT:", sum(trainlab), "/", len(trainlab))
        print("TEST STAT ", sum(testlab), "/", len(testlab))
        print("#" * 50)
        data = MultiIronyDataModule(train=ironyDataTrain, image_folder=parameters.imagefolder, test=ironyDataTest, val_data=ironyDataVal,
                               batch_size=parameters.batchsize)  # transform=data_transform)
        model = DualTransformerModel(pretrainedmodel, imodel, tokenizer, feature_extractor, lr=parameters.learning, opt=parameters.optimizer,
        ...
        for epoch in range(parameters.epochs):
            ...
            for j, data in enumerate(trainloader, 0):
                torch.cuda.empty_cache()
                inputs, images, labels = data['X'], data["I"], data['y'].to(model.dev)
                optimizer.zero_grad()
            ...
            with torch.no_grad():
                ...
                for k, data in enumerate(devloader, 0):
                    torch.cuda.empty_cache()
                    inputs, images, label = data['X'], data["I"], data['y'].to(model.dev)
                    dev_out = model(inputs, images)
                    if k == 0:
                        out = dev_out
                        log = label
                    else:
        ...
        with torch.no_grad():
            ...
            for x, batch in enumerate(test):
                data, ima, labels = batch['X'], batch["I"],batch['y'].to(model.dev)
                pred = model.predict_step(data, ima)
                if x == 0:
                    predictions = pred
                    groundTruth = labels
                else:
